...
public class EmployeeService {
    ...
    public Response addEmployee(Employees employee){
        ...
        if(findByEmployeeId(employee.getEmpId()).size()==0) {
            ...
            response.setMessage("employeeId: " + employee.getEmpId() + " data added");
            response.setResult(employee);
            return response;
        }else{
            response.setMessage("employee Details already exists");
            return response;
        }
    }
    ...
    public Response getEmployeeById(Long id){
        ...
        if (employees.isPresent()){
            response.setMessage("employeeId: "+id+" details are shown below");
            response.setResult(employees);
            return response;
        }else{
            response.setSuccess(false);
            response.setMessage("employeeId: "+id+" doesn't exist");
            return response;
        }
    }
    public Response getAllEmployee(){
       // Pageable pageable= PageRequest.of(0,2);
        ...
        Response response=new Response();
        response.setMessage("list of all employees shown below....");
        ...
        if (employees.isPresent()){
            ...
            response.setMessage("employeeId :"+id+" deleted");
            return response;
        }else{
            response.setSuccess(false);
            response.setMessage("employeeId: "+id+" doesn't exist");
    ...
    public Response updateEmployee(Employees employee){
        ...
        if (findByEmployeeId(employee.getEmpId()).size()==0){
            ...
            employeesRepo.save(employee);
            response.setMessage("employeeId: "+employee.getEmpId()+" updated");
            return response;
        }
    }
